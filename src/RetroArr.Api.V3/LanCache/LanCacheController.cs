...
namespace RetroArr.Api.V3.LanCache
{
    ...
    public class LanCacheController : ControllerBase
    {
        ...
        public LanCacheController(ConfigurationService configService, IHttpClientFactory httpClientFactory)
        {
        ...
        public async Task<IActionResult> Reconcile(CancellationToken ct)
        {
            ...
            try
            {
                ...
                var games = owned
                    ...
                    .Select(g => new { appId = g.AppId, name = g.Name, playtimeMinutes = g.PlaytimeForever })
                    .ToList();
                return Ok(new { steamConfigured = true, ownedCount = games.Count, games });
            }
            catch (Exception ex)
            {
